...
@Autonomous(name = "Test 12 red close - facing tag", group = "test")
public class close12redTest extends OpMode {
    ...
    // TEST: Shooting poses now face the AprilTag (117.6, 130)
    // shootPose1 (85, 88): angle to tag = atan2(42, 32.6) ≈ 52.4°
    private final Pose shootPose1 = new Pose(85, 88, Math.toRadians(52.4));
    ...
    // shootPose2 (87, 84): angle to tag = atan2(46, 30.6) ≈ 56.4°
    private final Pose shootPose2 = new Pose( 87, 84, Math.toRadians(56.4));
    ...
    // shootBall3 (87, 84): same as shootPose2 = 56.4°
    private final Pose shootBall3 = new Pose(87, 84, Math.toRadians(56.4));
    ...
    // shootBall4 (87, 84): same as shootPose2 = 56.4°
    private final Pose shootBall4 = new Pose(87, 84, Math.toRadians(56.4));
    ...
    public void statePathUpdate() {
        switch (pathState) {
            case start:
                ...
                // shootPose1 (85, 88) is ~53.17\" from tag ⇒ ~1683.0
                launcher.setVelocity(1683); // was 1850
                // Close-range hood: use CLOSE_HOOD_POSITION ≈ 0.2541
                hood.setPosition(0.2541); // was 0.25
                follower.setMaxPower(NORMAL_DRIVE_POWER);
                follower.followPath(shoot1);
                setPathState(close12redTest.PathState.actuallyshoot1);
                break;
                //
            case actuallyshoot1:
                ...
                if (!follower.isBusy() && pathTimer.getElapsedTimeSeconds()>2){
                    ...
                    if (pathTimer.getElapsedTimeSeconds()>3.5) {
                        setPathState(close12redTest.PathState.collection);
                    }
                }
                break;
            case gotocollect:
            ...
            case collection:
                ...
                if (!follower.isBusy()) {
                    ...
                    launcher.setVelocity(1690); // was 1850
                    hood.setPosition(0.2541);   // was 0.25
                    theWheelOfTheOx.setPower(1);
                    tree.setPower(1);
                    follower.followPath(collect1);
                    setPathState((close12redTest.PathState.goTowardsGate));
                }
                break;
            case goTowardsGate:
                ...
                if (!follower.isBusy() && goTowardsGateStarted && pathTimer.getElapsedTimeSeconds() > .5) {
                    setPathState(close12redTest.PathState.opengate);
                }
                break;
            case opengate:
                ...
                if (!follower.isBusy() && opengateStarted && pathTimer.getElapsedTimeSeconds() > 1) {
                    setPathState(close12redTest.PathState.shoot);
                }
                break;
            case shoot:
                // Continuously adjust based on limelight during shooting
                ...
                if (!follower.isBusy() && shoot2Started) {
                    ...
                    if(pathTimer.getElapsedTimeSeconds()>4) {
                        setPathState((close12redTest.PathState.collectAgain));
                    }
                }
                break;
            ...
            case collectAgain:
                if (!follower.isBusy()) {
                    ...
                    follower.setMaxPower(INTAKE_DRIVE_POWER);
                    setPathState((close12redTest.PathState.collectAgainEnd));
                }
                break;
            case collectAgainEnd:
                if (!follower.isBusy() && pathTimer.getElapsedTimeSeconds() > 1) {
                    launcher.setVelocity(1690); // was 1800
                    follower.followPath(collect2);
                    tree.setPower(1);
                    hood.setPosition(0.2541);   // was 0.25
                    theWheelOfTheOx.setPower(1);
                    setPathState((close12redTest.PathState.shootAgain));
                }
                break;
            case shootAgain:
                // Continuously adjust based on limelight during shooting
                ...
                if (!follower.isBusy() && shoot3Started) {
                    ...
                    {
                        setPathState((close12redTest.PathState.collectAgainAgain));
                    }
                }
                break;
            case collectAgainAgain:
                if (!follower.isBusy()) {
                    ...
                    setPathState((close12redTest.PathState.collectAgainAgainEnd));
                }
                break;
            case collectAgainAgainEnd:
                if (!follower.isBusy() && pathTimer.getElapsedTimeSeconds() > 2) {
                    ...
                    hood.setPosition(0.2541);   // was 0.25
                    theWheelOfTheOx.setPower(1);
                    setPathState((close12redTest.PathState.shootAgainAgain));
                }
                break;
            case shootAgainAgain:
                ...
                if (!follower.isBusy() && shoot4Started) {
                    ...
                    {
                        setPathState((close12redTest.PathState.parklol));
                    }
                }
                break;
            case parklol:
                ...
                if (!follower.isBusy() && parkingStarted && pathTimer.getElapsedTimeSeconds() > 2) {
                    setPathState((close12redTest.PathState.done));
                }
                break;
            case done:
                // Save final pose
                PoseStorage.savePose(follower.getPose());
                // TEST: Rotator stays at 0 since robot faces the tag
                rotator.setTargetPosition(0);
                PoseStorage.saveRotatorPosition(0);
                break;
    ...
    // TEST: Rotator adjustment removed - robot faces tag, rotator stays at 0
    public void adjustRotator(double tx) {
        // No adjustment needed - robot faces the tag, rotator stays at 0
        rotator.setTargetPosition(0);
    }
    ...
    /**
     * Updates limelight-based adjustments (velocity, hood) during shooting states
     * TEST: Rotator adjustment removed - robot faces tag, rotator stays at 0
     */
        ...
        if (limelight != null) {
            ...
            if (ll != null && ll.isValid()) {
                ...
                if (currentDistance > 0) {
                    ...
                    // TEST: No rotator adjustment - robot faces tag, rotator stays at 0
                    rotator.setTargetPosition(0);
