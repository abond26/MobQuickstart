...
//
//import static org.firstinspires.ftc.teamcode.util.PoseStorage.calculateRotatorPositionToFaceGoal;
//
...
//    private static final double BLUE_GOAL_X = 5;
...
//    private static final double SHOOT_DWELL_SEC = 1.5;
...
//
//    PathState pathState;
//    // Mirrored from red: blueX = 144 - redX, blueY = redY, blueHeading = Math.PI - redHeading
...
//    private final Pose collect2End2 = new Pose(10, 10, Math.toRadians(180));
//
//    private final Pose shootBall3 = new Pose(55, 15, Math.toRadians(113));
//    private final Pose collect3Start = new Pose(12, 12, Math.toRadians(180));
//    private final Pose shootBall4 = new Pose(55, 15, Math.toRadians(113));
//    private final Pose collect4Start = new Pose(12, 12, Math.toRadians(180));
//    private final Pose shootBall5 = new Pose(55, 15, Math.toRadians(113));
...
//                        }
//                        if (launcher.getVelocity() >= 1480 && launcher.getVelocity() <= 1520) {
...
//        rotator.setPower(1.0); // full power so rotator moves quickly to aim
//
...
//        //int RotatorSave = calculateRotatorPositionToFaceGoal(rotator.getTargetPosition());
//        statePathUpdate();
...
//            telemetry.addData("rotator target ticks", getRotatorTargetTicksForBlueGoal());
...
//    }
//    /**
//     * Turret angle (deg) to aim at blue goal.
//     * Same flow as SubsysTele: ChassisLocal.calculateTurretAngle(target) then Turret.setRotatorToAngle(angle).
//     */
...
//    }
//
//    /** Target ticks for blue goal (Turret subsystem uses TurretConstants.rotator180Range). */
...
//    }
//
//    /** Aims the rotator at the blue goal. */
//    private void aimRotatorAtBlueGoal() {
//        if (turret == null) return;
//        turret.setRotatorToAngle(getTurretAngleDegForBlueGoal());
//    }
//
//    /** True if the rotator is aimed at the blue goal within tolerance (so safe to open blocker and feed). */
...
//    }
//
//    /** Blue side: rotator adjustment uses + offset (mirrored from red's - offset). Uses Turret for position. */
